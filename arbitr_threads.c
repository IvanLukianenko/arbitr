...
2. будет регистрировать каждого подключившегося абонента к очереди:
  2.1. Пользователь присылает свое имя, по этому имени вычисляется индентификатор
3. При передачи сообщения, пользователь указывает имя абонента и посылаемое ему сообщение, арбитр вычисляет его идентификтор и если этот пользователь в сети, отправляет, адресованное ему сообщение.
4. Проверяет онлайн, каждый клиент присылает каждые 10 секунд сообщение, арбитр обновляет таблицу времени в сети.
5. Если пользователь дольше 20 секунд не присылал уведомление об онлайне, то он удаляется из всех таблиц.
*/
#include <sys/types.h>
#include <sys/ipc.h>
#include <sys/msg.h>
